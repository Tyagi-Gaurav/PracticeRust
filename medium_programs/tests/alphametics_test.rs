...
fn assert_alphametic_solution_eq(puzzle: &str, solution: &[(char, u8)]) {
    let answer = medium_programs::alphametics::solve(puzzle);
...
fn must_have_unique_value_for_each_letter() {
    let answer = medium_programs::alphametics::solve("A == B");
    assert_eq!(answer, None);
}

#[test]
fn leading_zero_solution_is_invalid() {
    let answer = medium_programs::alphametics::solve("ACA + DD == BD");
    assert_eq!(answer, None);
}

#[test]
fn sum_must_be_wide_enough() {
    let answer = medium_programs::alphametics::solve("ABC + DEF == GH");
    assert_eq!(answer, None);
}
